...
import pytesseract
# Wrap pdf2image import to handle Poppler-not-found errors gracefully
...
try:
    ...
    # pdf2image may fail to import if Poppler is not found
    def convert_from_path(*args, **kwargs):
        raise RuntimeError(
            f"pdf2image is not available (Poppler not found).\n"
            f"Original error: {pdf2image_err}\n"
            f"Install Poppler for Windows and set POPPLER_PATH environment variable.\n"
...
def _ocr_configs_fast(img: Image.Image) -> List[str]:
    ...
    cfgs = [
        ...
        r"--oem 1 --psm 12 -c preserve_interword_spaces=1",
...
def normalize_for_dates(s: str) -> str:
    ...
    # Handle multi-char/Mojibake replacements manually
    s = s.replace('â€”', '-')
    s = s.replace('â€“', '-')
    s = s.replace('â€š', ',')
    return s
    ...
    *,
    ...
    if save_raw_path:
        try:
            with open(save_raw_path, 'w', encoding='utf-8') as f:
                json.dump(raw_data, f, indent=2, ensure_ascii=False)
            logger.info("âœ“ Saved raw JSON output to: %s", save_raw_path)
        except Exception as e:
            logger.error("âœ— Failed to save JSON output: %s", e)
